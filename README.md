...
- **球权状态**：红球仍在时按 `Q` 下一杆改瞄彩球（任选，一杆后自动回落；脉冲由球位位移判定，遮挡/丢帧/检测抖动都不会让任选态泄漏或提前失效），`W` 强制切回红球（`O` 兼容）；红球清完后无条件严格清彩（黄→绿→棕→蓝→粉→黑），最后一颗红后想挑其他彩球用 `G` 手动点选；
  清彩目标带跨帧滞回：低分彩球瞬时漏检（球杆/文字遮挡几帧）不会跳到下一颗，
  连续消失确认落袋才移交，更低分球稳定重现会按规则夺回目标
- **台面锁定**（`TableTracker`）：首帧检测后锁定角点，周期重检；单边误检、
  渐进漂移和短暂漏检保持原框，只有连续出现同一刚性偏移才接受窗口移动。
  捕获帧同时绑定区域坐标，避免重新框选时旧帧把白框带偏；袋口也在台面坐标中锁定，
  消除逐帧四边形/袋口抖动
...
`Q` 进红后下一杆打彩球（O 兼容；清彩阶段无效） · `W` 强制切回红球 · `V` 显示/隐藏鬼球标记 ·
`T` 隐藏 · `C` 重识别 · `Esc` 退出
...
3. **规则状态不是完整游戏状态**：截图无法可靠判断失误、换手和球是否入袋；红球仍在时按 `Q`
   （`O` 兼容）切彩球，按 `W` 可强制切回红球，避免因错误状态推荐犯规球。
