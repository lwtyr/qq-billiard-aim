...
- **决策层**（`aimtool/snooker.py`）：红球阶段选最好打的红球；清彩阶段按
  黄(2)→绿(3)→棕(4)→蓝(5)→粉(6)→黑(7) 分值顺序
- **开局解球回退**：完整红球架没有安全入袋路线时，明确显示母球方向最外层
  红球的碰撞点；该线是撞散球架提示，不伪装成入袋路线
- **球权状态**：最后一颗红球落袋后保留“红后选彩”阶段；无法由截图判断的换手/犯规可按 `O` 手动切换
- **台面锁定**（`TableTracker`）：首帧检测后 EMA 平滑角点、周期重检，
  小噪声保持原框，大跳变连续确认后才接受；袋口也在台面坐标中锁定，
  消除逐帧四边形/袋口抖动
- **自动路线**：遍历六个袋口、直球和一库/两库路线，过滤障碍后按切角容错、
  反弹次数和总路程评分，自动选当前更容易进球的一条；`route_options` 保留备选路线；
...
3. **规则状态不是完整游戏状态**：截图无法可靠判断失误、换手和球是否入袋；按 `O`
   手动切换球权，避免因错误状态推荐犯规球。
